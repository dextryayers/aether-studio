...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6 lg:space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 lg:gap-4">
        {cards.map((card) => (
          <Link
            key={card.label}
            to={card.href}
            className="relative bg-[#0a0a0a] border border-zinc-800/60 p-4 lg:p-6 hover:border-primary/30 transition-all duration-300 group block"
          >
            ...
            <div className="relative space-y-3 lg:space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {loaded ? (
                    <span className="text-xl lg:text-2xl font-black tracking-tighter text-white tabular-nums">
                      {card.value}
                    </span>
                  ) : (
                    <span className="w-8 h-6 bg-zinc-800 animate-pulse" />
                  )}
                </div>
              </div>
              <div>
                <p className="text-[10px] lg:text-xs font-bold uppercase tracking-widest text-zinc-400 group-hover:text-primary transition-colors">
                  {card.label}
                </p>
                <p className="text-[8px] lg:text-[9px] text-zinc-600 mt-0.5 lg:mt-1 font-medium">{card.desc}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1 text-[8px] lg:text-[9px] font-bold uppercase tracking-wider text-zinc-600 group-hover:text-primary transition-colors">
                  Manage
                  <ArrowRight className="h-2.5 w-2.5 lg:h-3 lg:w-3 transition-transform duration-300 group-hover:translate-x-1" />
                </span>
                {card.addHref && (
                  <Link to={card.addHref} onClick={(e) => e.stopPropagation()}
                    className="ml-auto relative z-10 flex items-center gap-1 h-8 px-4 border border-zinc-700/60 text-[9px] font-bold uppercase tracking-wider text-zinc-400 hover:text-primary hover:border-primary/50 hover:bg-primary/5 transition-all">
      ...
      {/* Quick actions */}
      <div className="border-t border-zinc-800/30 pt-6 lg:pt-8">
        <p className="text-[9px] lg:text-[10px] font-bold uppercase tracking-[0.3em] text-zinc-600 mb-3 lg:mb-4">
          Quick Actions
        </p>
        <div className="flex flex-nowrap lg:flex-wrap gap-2 lg:gap-3 overflow-x-auto pb-2 lg:pb-0 -mx-4 lg:mx-0 px-4 lg:px-0 scrollbar-none">
          {[
            ...
            <Link
              key={action.label}
              to={action.href}
              className="shrink-0 h-9 px-4 lg:px-5 border border-zinc-800 text-[9px] font-bold uppercase tracking-widest text-zinc-400 hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-all flex items-center gap-2"
            >
              + {action.label}
            </Link>
          ))}
