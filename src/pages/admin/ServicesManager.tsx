    ...
    <div className="space-y-5 lg:space-y-8">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl lg:text-3xl font-black tracking-tighter uppercase text-white truncate">Services</h1>
          <p className="text-[9px] lg:text-[10px] text-zinc-500 font-medium mt-0.5 lg:mt-1 tracking-wide">{services.length} {services.length === 1 ? "service" : "services"}</p>
        </div>
      ...
      {showForm && (
        <div className="border border-zinc-800/60 bg-[#0a0a0a] p-4 lg:p-6 space-y-4 lg:space-y-5 relative">
          <div className="flex items-center justify-between">
            <h2 className="text-[10px] lg:text-xs font-bold uppercase tracking-widest text-zinc-400">{editing ? "Edit Service" : "New Service"}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
            ...
            <div className="space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Icon (lucide)</label>
              <input value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} className={input} placeholder="e.g. Globe" />
            </div>
            <div className="space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Tags (comma separated)</label>
            ...
            <div className="md:col-span-2 space-y-1 lg:space-y-1.5">
              ...
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                className="w-full h-20 lg:h-24 bg-[#050505] border border-zinc-800 px-3 py-2 text-sm text-white outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/20 transition-all placeholder:text-zinc-700 resize-none" placeholder="Describe the service..." />
            </div>
          </div>
      ...
      {services.length === 0 ? (
        <div className="border border-zinc-800/50 p-8 lg:p-12 text-center">
          <div className="w-10 h-10 lg:w-12 lg:h-12 mx-auto mb-3 lg:mb-4 border border-zinc-800 flex items-center justify-center">
            <Wrench className="h-4 w-4 lg:h-5 lg:w-5 text-zinc-600" />
          </div>
          <p className="text-xs lg:text-sm font-bold text-zinc-500">No services yet</p>
          <p className="text-[9px] lg:text-[10px] text-zinc-700 mt-1 mb-5">Add your first service offering</p>
          <button onClick={openNew} className="h-9 px-5 bg-primary text-black text-[9px] font-black uppercase tracking-[0.25em] hover:brightness-110 transition-all">+ Create Service</button>
        </div>
      ) : (
        <div className="space-y-1.5 lg:space-y-2">
          {services.map((s) => (
            <div key={s.id} className="flex items-center justify-between bg-[#0a0a0a] border border-zinc-800/40 px-3 lg:px-5 py-3 lg:py-4 hover:border-zinc-700/60 transition-all gap-2">
              <div className="min-w-0">
                <p className="text-xs lg:text-sm font-bold text-white">{s.title}</p>
                {s.tags?.length > 0 && (
              ...
              <div className="flex items-center gap-1 lg:gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity shrink-0">
                <button onClick={() => openEdit(s)} className="p-1.5 lg:p-2 text-zinc-600 hover:text-primary hover:bg-primary/10 transition-all"><Pencil className="h-3 w-3 lg:h-3.5 lg:w-3.5" /></button>
                <button onClick={() => remove(s.id)} className="p-1.5 lg:p-2 text-zinc-600 hover:text-red-400 hover:bg-red-500/10 transition-all"><Trash2 className="h-3 w-3 lg:h-3.5 lg:w-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
