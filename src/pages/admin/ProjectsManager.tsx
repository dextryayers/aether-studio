    ...
    <div className="space-y-5 lg:space-y-8">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl lg:text-3xl font-black tracking-tighter uppercase text-white truncate">Projects</h1>
      ...
      {showForm && (
        <div className="border border-zinc-800/60 bg-[#0a0a0a] p-4 lg:p-6 space-y-4 lg:space-y-5 relative">
          <div className="flex items-center justify-between">
            <h2 className="text-[10px] lg:text-xs font-bold uppercase tracking-widest text-zinc-400">
              {editing ? "Edit Project" : "New Project"}
            </h2>
            <button onClick={closeForm} className="text-zinc-600 hover:text-zinc-300 transition-colors"><X className="h-4 w-4" /></button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
            <div className="space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Title</label>
              <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className={input} placeholder="Project name" />
            </div>
            <div className="space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Category</label>
              <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
                className="w-full h-10 bg-[#050505] border border-zinc-800 px-3 text-sm text-white outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/20 transition-all appearance-none cursor-pointer">
                <option value="" disabled className="text-zinc-600">Select category</option>
                <option value="Tools" className="text-white">Tools</option>
                <option value="Web Fullstack" className="text-white">Web Fullstack</option>
                <option value="Front End Web" className="text-white">Front End Web</option>
              </select>
            </div>
            <div className="space-y-1 lg:space-y-1.5">
            ...
            <div className="space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Repo URL</label>
              <input value={form.repo_url} onChange={(e) => setForm({ ...form, repo_url: e.target.value })} className={input} placeholder="https://github.com/..." />
            </div>
            <div className="space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Demo URL</label>
            ...
            <div className="md:col-span-2 space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Description</label>
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                className="w-full h-20 lg:h-24 bg-[#050505] border border-zinc-800 px-3 py-2 text-sm text-white outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/20 transition-all placeholder:text-zinc-700 resize-none" placeholder="Describe the project..." />
            </div>
            <div className="md:col-span-2 space-y-1 lg:space-y-1.5">
              <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-zinc-500">Tech Stack (comma separated)</label>
      ...
      {projects.length === 0 ? (
        <div className="border border-zinc-800/50 p-8 lg:p-12 text-center">
          <div className="w-10 h-10 lg:w-12 lg:h-12 mx-auto mb-3 lg:mb-4 border border-zinc-800 flex items-center justify-center">
            <FolderKanban className="h-4 w-4 lg:h-5 lg:w-5 text-zinc-600" />
          </div>
          <p className="text-xs lg:text-sm font-bold text-zinc-500">No projects yet</p>
          <p className="text-[9px] lg:text-[10px] text-zinc-700 mt-1 mb-5">Create your first project to get started</p>
          <button onClick={openNew} className="h-9 px-5 bg-primary text-black text-[9px] font-black uppercase tracking-[0.25em] hover:brightness-110 transition-all">+ Create Project</button>
        </div>
      ) : (
        <div className="space-y-1.5 lg:space-y-2">
          {projects.map((p) => (
            <div key={p.id} className="flex items-center justify-between bg-[#0a0a0a] border border-zinc-800/40 px-3 lg:px-5 py-3 lg:py-4 hover:border-zinc-700/60 transition-all gap-2">
              <div className="flex items-center gap-3 lg:gap-4 min-w-0">
                <div className="w-8 h-8 lg:w-10 lg:h-10 bg-zinc-800/50 border border-zinc-700/30 flex items-center justify-center shrink-0">
                  {p.image ? <img src={p.image} alt="" className="w-full h-full object-cover" /> : <FolderKanban className="h-3 w-3 lg:h-4 lg:w-4 text-zinc-600" />}
                </div>
                <div className="min-w-0">
                  <p className="text-xs lg:text-sm font-bold text-white truncate">{p.title}</p>
                  <div className="flex items-center gap-2 lg:gap-3 mt-0.5 flex-wrap">
                    ...
                    {p.year && <span className="text-[8px] lg:text-[9px] font-mono text-zinc-600">{p.year}</span>}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-1 lg:gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity shrink-0">
                <button onClick={() => openEdit(p)} className="p-1.5 lg:p-2 text-zinc-600 hover:text-primary hover:bg-primary/10 transition-all"><Pencil className="h-3 w-3 lg:h-3.5 lg:w-3.5" /></button>
                <button onClick={() => remove(p.id)} className="p-1.5 lg:p-2 text-zinc-600 hover:text-red-400 hover:bg-red-500/10 transition-all"><Trash2 className="h-3 w-3 lg:h-3.5 lg:w-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
