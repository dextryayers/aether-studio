...
export default function ContactManager() {
  ...
  const save = async () => {
    ...
    catch { setJsonError(true); toastError("Invalid JSON in social links"); return; }
    setSaving(true);
    try { await api.updateContact({ ...form, social_links }); toastSuccess("Contact info saved"); }
    ...
    return (
      <div className="space-y-5 lg:space-y-8">
        ...
        <div className="border border-border/50 bg-card p-4 lg:p-6 space-y-4 lg:space-y-5">
          <div className="grid grid-cols-2 gap-3 lg:gap-4"><div className="h-10 bg-muted animate-pulse" /><div className="h-10 bg-muted animate-pulse" /></div>
  ...
  return (
    <div className="space-y-5 lg:space-y-8">
      <div>
        <h1 className="text-xl lg:text-3xl font-black tracking-tighter uppercase text-foreground">Contact Info</h1>
        <p className="text-[9px] lg:text-[10px] text-muted-foreground font-medium mt-0.5 lg:mt-1 tracking-wide">Manage your contact details and social links</p>
      </div>

      <div className="border border-border/60 bg-card p-4 lg:p-6 space-y-4 lg:space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
          <div className="space-y-1 lg:space-y-1.5">
            <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-muted-foreground">Email</label>
            <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className={input} placeholder="hello@example.com" />
          </div>
          <div className="space-y-1 lg:space-y-1.5">
            <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-muted-foreground">Phone</label>
            <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className={input} placeholder="+62 ..." />
          </div>
        </div>
        <div className="space-y-1 lg:space-y-1.5">
          <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-muted-foreground">Address</label>
          <input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className={input} placeholder="City, Country" />
        </div>
        <div className="space-y-1 lg:space-y-1.5">
          <label className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-muted-foreground">Social Links (JSON)</label>
          <textarea value={form.social_links} onChange={(e) => { setForm({ ...form, social_links: e.target.value }); setJsonError(false); }}
            className={`w-full h-28 lg:h-36 bg-background border px-3 lg:px-4 py-2 lg:py-3 text-sm font-mono text-foreground outline-none transition-all resize-none ${
              jsonError ? "border-red-500/50" : "border-border focus:border-primary/60 focus:ring-1 focus:ring-primary/20"
            }`} placeholder='{&#10;  "github": "https://...",&#10;  "linkedin": "https://..."&#10;}' />
