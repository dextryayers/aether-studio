...
const iconMap: Record<string, React.ElementType> = { Code2, Palette, Search, Smartphone }

const fallbackServices = [
  { titleKey: "servicesSection.items.s1.title", descKey: "servicesSection.items.s1.desc", icon: "Palette" },
  { titleKey: "servicesSection.items.s2.title", descKey: "servicesSection.items.s2.desc", icon: "Code2" },
  { titleKey: "servicesSection.items.s3.title", descKey: "servicesSection.items.s3.desc", icon: "Smartphone" },
  { titleKey: "servicesSection.items.s4.title", descKey: "servicesSection.items.s4.desc", icon: "Search" },
]

export function ServicesSection() {
  const { t } = useLanguage()
  const [services, setServices] = useState<{ title: string; description: string; icon: React.ElementType }[]>([])
  ...
  useEffect(() => {
    getServices()
      ...
      .catch(() => {
        setServices(fallbackServices.map((s) => ({
          title: t(s.titleKey),
